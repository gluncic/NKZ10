...
def skupine_rod(request: Request, rod: str):
    ...
    for skupina in skupine_lista:
        ...
        html += f"<li><a href='javascript:void(0)' hx-get='/zanimanja/{skupina_enc}' hx-target='#skupina-{skupina_enc}' hx-swap='outerHTML'>{skupina}</a></li>"
    html += "</ul></div>"
    return html
...
def zanimanja_skupina(request: Request, skupina: str):
    ...
    for z in zanimanja:
        sifra = z['sifra']
        ime = z['ime']
        html += f"<li><a href='javascript:void(0)' hx-get='/toggle/{sifra}' hx-target='#blok-{sifra}' hx-swap='outerHTML'>{ime}</a></li>"
    html += "</ul></div>"
    return html
...
def sakrij_skupinu(request: Request, skupina: str):
    ...
    skupina_enc = quote(skupina)
    html = f"<div class='blok' id='skupina-{skupina_enc}'>"
    html += f"<a href='javascript:void(0)' hx-get='/zanimanja/{skupina_enc}' hx-target='#skupina-{skupina_enc}' hx-swap='outerHTML'>{skupina}</a>"
    html += "</div>"
    return html
...
def toggle_opis(request: Request, code: str):
    ...
    html += f"<a href='javascript:void(0)' hx-get='/sakrij/{code}' hx-target='#blok-{code}' hx-swap='outerHTML'>{ime}</a>"
...
def sakrij_opis(request: Request, code: str):
    ime = zan_dict[code]["ime"]
    html = f"<div class='zanimanje-blok' id='blok-{code}'>"
    html += f"<a href='javascript:void(0)' hx-get='/toggle/{code}' hx-target='#blok-{code}' hx-swap='outerHTML'>{ime}</a>"
    html += "</div>"
    return html
